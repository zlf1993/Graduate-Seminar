...
if __name__=='__main__':
    ...
    max_acc=-1.0
    svm_acc = tf.Variable(0.0, name='svm_acc')
    svm_acc2 = tf.add(svm_acc, 0)
    svm_acc_summary = tf.summary.scalar('svm_accuracy_prediction', svm_acc2)
    sess = tf.Session()

    merged = tf.summary.merge_all()
    test_summary_writer = tf.summary.FileWriter('./test_acc', sess.graph)
    ...
    for cs in cellsize:
        ...
        for lr in learning_rate:
            for rs in regularization_strength:
                ...
                my_merged, my_acc2= sess.run([merged, svm_acc2], feed_dict={svm_acc: acc})
                test_summary_writer.add_summary(my_merged, i)
                i += 1
                #选取精度最大时的最优模型
                if(acc>max_acc):
                    max_acc=acc
